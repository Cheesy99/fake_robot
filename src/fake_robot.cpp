...
public:
  LandmarkPublisher() : Node("landmark_publisher") {
    ...
    landmarks_publisher_ =
        this->create_publisher<sensor_msgs::msg::PointCloud2>("landmarks_gt",
                                                              10);
    robot_publisher_ =
        this->create_publisher<nav_msgs::msg::Odometry>("robot_gt", 10);
    robot_noisy_publisher_ =
        this->create_publisher<nav_msgs::msg::Odometry>("robot_noisy", 10);
    observed_landmarks_publisher_ =
    ...
    this->declare_parameter("observation_radius",
                            10.0); // Radius around robot to observe landmarks
    this->declare_parameter("measurement_noise_variance",
                            0.1); // Gaussian noise variance
    ...
    measurement_noise_variance_ =
        this->get_parameter("measurement_noise_variance").as_double();
    robot_noise_variance_x_ =
        this->get_parameter("robot_noise_variance_x").as_double();
    robot_noise_variance_y_ =
        this->get_parameter("robot_noise_variance_y").as_double();
    robot_noise_variance_theta_ =
        this->get_parameter("robot_noise_variance_theta").as_double();

    RCLCPP_INFO(this->get_logger(),
                "Circle parameters - radius: %.2f m, speed: %.2f m/s, center: "
                "(%.2f, %.2f)",
                circle_radius_, circle_speed_, circle_center_x_,
                circle_center_y_);
    RCLCPP_INFO(this->get_logger(),
                "Observation parameters - radius: %.2f m, noise variance: %.4f",
                observation_radius_, measurement_noise_variance_);
    RCLCPP_INFO(
        this->get_logger(),
        "Robot noise parameters - x_var: %.4f, y_var: %.4f, theta_var: %.4f",
        robot_noise_variance_x_, robot_noise_variance_y_,
        robot_noise_variance_theta_);
    ...
    std::string landmarks_file =
        this->get_parameter("landmarks_file").as_string();

    RCLCPP_DEBUG(this->get_logger(), "Landmarks file: %s",
                 landmarks_file.c_str());

    // Load landmarks from file
    if (!load_landmarks(landmarks_file)) {
      RCLCPP_ERROR(this->get_logger(), "Failed to load landmarks from file: %s",
                   landmarks_file.c_str());
      return;
    }
    ...
    // Initialize both GT and noisy robot pose to same starting location on
    // circle At t=0: theta=0, position on circle at theta=0
...
private:
  ...
  std::normal_distribution<double> noise_dist_{
      0.0, 1.0}; // Will be scaled by std dev
    ...
    while (std::getline(file, line)) {
      ...
      if (pos1 == std::string::npos || pos2 == std::string::npos) {
        RCLCPP_WARN(this->get_logger(),
                    "Invalid line format (expected id,x,y): %s", line.c_str());
        continue;
      }
      ...
      try {
        ...
        RCLCPP_WARN(this->get_logger(), "Failed to parse line: %s (error: %s)",
                    line.c_str(), e.what());
        continue;
      }
    }
  ...
  void publish_landmarks() {
    ...
    pc->point_step =
        16; // 4 bytes (x) + 4 bytes (y) + 4 bytes (z) + 4 bytes (id)
  ...
  void publish_robot_trajectory() {
    ...
    // Compute previous GT pose for velocity (if first step, previous ==
    // current)
    ...
    odom.twist.twist.linear.x =
        traveled_distance * std::cos(motion_direction) / dt_;
    odom.twist.twist.linear.y =
        traveled_distance * std::sin(motion_direction) / dt_;
  ...
  void publish_robot_noisy_pose() {
    ...
    double distance_noise =
        noise_dist_(rng_) * std::sqrt(robot_noise_variance_x_) * distance_scale;
    double theta_noise = noise_dist_(rng_) *
                         std::sqrt(robot_noise_variance_theta_) *
                         distance_scale;
    ...
    // Update noisy pose (noise accumulates along motion direction and
    // orientation)
